"""S9-T02: Stage9 new-edition Paragraph formatter (V1.8 #47 / #30; V2.5 tuned).

Implements STAGE9-PLAN S9-T02, formatter half (V1.8 ``# 47`` Natural
Paragraph rule order + ``# 30`` render profile layering):

* ``FORMATTER_VERSION`` (``para-v2.5``) + ``PARA_PARAMS_V2`` (versioned
...
V2.5 tuning (P0-1, params/rules only, no architecture change, no LLM):

* ``target_chars`` 240 -> 120 (about half: shorter paragraphs; weak
  punctuation such as ``，。、；`` keeps splitting at target via the
  frozen engine third rule, now reached twice as fast).
* ``hard_max_chars`` 400 -> 200 (X=200: V2.5 guarantees NO paragraph
  longer than 200 chars; the frozen engine only breaks *between*
  segments, so ``render_with_v2`` adds a pure post-pass that splits
  any over-long paragraph *inside* at weak punctuation, else hard-cut
  at 200).
* ``min_paragraph_chars`` = 40 (new): a pure post-pass merges
  sub-40-char fragments with a neighbour (never exceeding the 200
  hard cap) so short videos are not shattered into碎渣.
...
FORMATTER_VERSION = "para-v2.5"

# The frozen formatter this edition supersedes.
BASE_FORMATTER_VERSION = "para-v1"

# Versioned fixed values (V2.5 tuned; X=200 hard cap, MIN=40 anti-shatter).
# Versus frozen para-v1 {1.5, 180, 400}: target 180 -> 120 (shorter
# paragraphs, weak punctuation splits sooner), hard_max 400 -> 200
# (no paragraph may exceed 200 chars after the wrapper post-pass).
# Versus para-v2 {1.5, 240, 400}: target about half (240 -> 120).
# pause stays put. min_paragraph_chars rides inside the profile so a
# MIN change also mints a new Render Revision (Case 5); the frozen
# engine ignores the extra key (it reads only the three above).
PARA_PARAMS_V2 = {
...
# for the V2.5 hard-cap post-pass (covers，。、；：plus the strong set).
WEAK_SPLIT_PUNCT = (
    "。", "！", "？", "…", "，", "、", "；", "：",
    "!", "?", ".", ",", ";", ":",
)
...
def _enforce_hard_cap(paragraphs: list, hard_max: int) -> list:
    """Post-pass 1 (V2.5): guarantee no paragraph exceeds hard_max."""
    out: list[str] = []
...
def _merge_shorts(paragraphs: list, min_chars: int,
                  hard_max: int) -> list:
    """Post-pass 2 (V2.5): merge sub-min fragments, anti-shatter.
...
def render_with_v2(segments: list) -> list:
    ...
    V2.5: frozen engine (target 120 / hard_max 200) + hard-cap
    post-pass (X=200, weak-punctuation-first intra splits) + short
    merge post-pass (MIN=40, anti-shatter for short videos).
    """
...
def check_rule_order() -> dict:
    ...
    untouched stage3 engine. V2.5 adds two wrapper probes (hard cap
    X=200, MIN=40 anti-shatter) over ``render_with_v2``.
    """
    ...
    # 5. V2.5 hard cap (X=hard_max): a single over-long segment with no
    ...
    # 6. V2.5 anti-shatter (MIN): pause-split crumbs merge back instead
    #    of staying碎渣.
    paras = render_with_v2(
        [
            {"id": "m%d" % i, "text": "ab",
    ...
    probes["min_merge_v25"] = len(paras) == 1
